...
v[1]=VS(c,"IdGood");
cur1=ADDOBJ("Firm");
cur3=ADDOBJS(c,"sFirm");
cur3->hook=cur1;
cur1->hook=cur3; 
WRITELS(cur1,"Age",0, t);
WRITELS(cur1,"Visibility",0.01, t);
WRITES(cur1,"IdFirm",v[22]=V("CounterIdFirm"));
WRITES(cur3,"sIdFirm",v[22]);
...
v[30]=VS(c,"IdNeed");
cur4=SEARCH_CND("IdGood",v[30]); // search for the sector that produces the good that satisfies the need that the iteration is buying

CYCLES(cur4, cur5, "sFirm")
{
 cur=cur5->hook;
 v[24]=1; //assume the option to be viable
 //cur3=SEARCH_CNDS(cur,"IdPNeed",v[30]);
  if(RND>VLS(cur,"Visibility",1))
 ...
 {cur7=SEARCH("Bank");
  ...
  CYCLES(cur4, cur1, "Firm")
   {// run a first cycle trough firms to set the number of labor in t-1
    ...
    if(VS(cur6,"sIdFirm")!=0)
     cur6=ADDOBJS(cur5,"sFirm");;
    WRITES(cur6,"sIdFirm",VS(cur1, "IdFirm"));
    cur6->hook=cur1;
    cur1->hook=cur6;
